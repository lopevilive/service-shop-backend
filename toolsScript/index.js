...
module.exports.vipExpiredHandle = async (expiredDays, isDel = false) => {
  ...
  queryBuild.andWhere('(Shop.mode & 1) = 0'); // 位运算：第0位为0（表示尚未处理过过期）
  ...
  for (const shopItem of shopList) {
    ...
    if (isDel) {
      ...
      // B. 更新店铺自身的 mode 标识位 (第0位置为1)
      const newMode = shopItem.mode | (1 << 0);
...
const init = async () => {
  setTimeout(() => {
    ...
    // this.vipExpiredHandle(30) // 处理过期会员，会把产品mode 置 1
